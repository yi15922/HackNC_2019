...
server <- function(input, output, session) {
  ...
  output$uiStub <- renderUI(
    ...
    fluidPage(                                  #     move the ui into the server function
      ...
      fluidRow(
        column(12,
               HTML("<div align = 'center'><h3><a href='?home'>Home</a> | ",
                    "<a href='?explore'>Data Exploration</a> |",
                    "<a href='https://yi15922.shinyapps.io/data-dashboard' target='_blank'>Interactive Visualization</a>",
                    "</h3></div>")
        )
      ),
  ...
  validFiles = c("home.R",                             # valid files must be hardcoded here
                 "explore.R",
                 "interact.R")                     #    for security (use all lower-case
